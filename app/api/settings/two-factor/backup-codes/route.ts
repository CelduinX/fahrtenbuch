...
import { regenerateBackupCodes } from "@/lib/repositories/two-factor";
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await regenerateBackupCodes(user.id, input.currentPassword, input.code));
  } catch (error) {
